...
import issueDescriptions from "../Descriptions/SecurityIssueDescriptions.js";
import IssueDetailModal from "../IssueModal/VulnerabilityModal";

/**
 * Renders the security analysis results.
 *
 * @param {Object} props - The component props.
 * @param {Object} props.data - The data containing security analysis results.
 * @returns {JSX.Element} The rendered component.
 */
function SecurityResults({ data }) {
  ...
  // Function to extract line number from a vulnerability string
  const extractLineNumber = (vulnerability) => {
    const match = vulnerability.match(/Violation at line (\d+)/);
  ...
  return (
    <div className="custom-results-container">
      ...
      {/* Display stats */}
      {typeof vulnerabilitiesCount === "number" && (
        <div className="result-card">
          <h2>Statistics</h2>
          <p>Total Violations: {vulnerabilitiesCount}</p>
        </div>
      )}
